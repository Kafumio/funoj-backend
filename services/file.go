...
func (svc *FileServiceImpl) StartUpload() (string, *e.Error) {
	tempPath := utils.GetTempDir(svc.config)
...
func (svc *FileServiceImpl) Upload(path string, ctx *gin.Context, file *multipart.FileHeader) *e.Error {
...
func (svc *FileServiceImpl) CheckChunkSet(path string) ([]string, *e.Error) {
...
// CancelUpload 取消上传
func (svc *FileServiceImpl) CancelUpload(path string) *e.Error {
...
func (svc *FileServiceImpl) CompleteUpload(p string, fileName string, h string, hashType string) *e.Error {
	// 读取path中的所有文件
